...
async def publish_channel_menu(message: Message):
    ...
    await message.answer(
        "✅ Меню заново опубликовано в канале.\n"
        "Теперь закрепите новое сообщение вручную."
    )
...
@dp.message(F.text.in_({"/меню", "/закреп"}))
async def publish_menu_russian_command(message: Message):
    await publish_channel_menu(message)
