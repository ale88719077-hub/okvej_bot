...
BOT_VERSION = "16.1"
BOT_BUILD = "2026-07-18-mini-app-design"
...
<div id="app"></div>
<style>
:root{color-scheme:light dark;--blue:#1597dc;--blue2:#0f7fc2;--line:#dce8ef;--muted:#6c7b86;--card:var(--tg-theme-secondary-bg-color,#fff);--bg:var(--tg-theme-bg-color,#f4f8fb);--text:var(--tg-theme-text-color,#16212b)}
*{box-sizing:border-box}body{margin:0;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:var(--bg);color:var(--text);-webkit-tap-highlight-color:transparent}button,input,textarea{font:inherit}.shell{max-width:760px;margin:auto;padding:10px 10px 104px}.hero{background:linear-gradient(135deg,#54c3f1 0%,#1597dc 58%,#0f7fc2 100%);color:#fff;padding:16px;border-radius:20px;margin-bottom:10px;display:flex;align-items:center;justify-content:space-between;gap:12px}.brand{display:flex;align-items:center;gap:10px}.logo{width:46px;height:46px;border-radius:15px;background:rgba(255,255,255,.2);display:grid;place-items:center;font-size:26px}.hero h1{margin:0;font-size:22px}.hero p{margin:3px 0 0;font-size:13px;opacity:.9}.hero-stat{font-size:12px;background:rgba(255,255,255,.18);padding:7px 9px;border-radius:999px;white-space:nowrap}.toolbar{position:sticky;top:0;z-index:3;background:var(--bg);padding:2px 0 8px}.search-wrap{position:relative}.search{width:100%;padding:12px 42px 12px 14px;border:1px solid var(--line);border-radius:14px;background:var(--card);color:inherit;outline:none}.search:focus{border-color:var(--blue)}.search-icon{position:absolute;right:13px;top:10px;opacity:.55}.chips{display:flex;gap:7px;overflow:auto;padding:8px 0 1px;scrollbar-width:none}.chips::-webkit-scrollbar{display:none}.chip{border:0;border-radius:999px;padding:8px 12px;background:#e8f5fc;color:#1679b6;white-space:nowrap;font-size:13px}.chip.active{background:var(--blue);color:#fff}.section-head{display:flex;justify-content:space-between;align-items:center;margin:8px 2px 10px}.section-head h2{font-size:17px;margin:0}.count-label{font-size:13px;color:var(--muted)}.grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.card{position:relative;background:var(--card);border-radius:17px;overflow:hidden;border:1px solid rgba(20,90,120,.07);display:flex;flex-direction:column;min-width:0}.photo-wrap{position:relative;background:#fff;aspect-ratio:1/1}.photo{width:100%;height:100%;object-fit:contain;display:block}.fav{position:absolute;right:7px;top:7px;border:0;width:34px;height:34px;border-radius:50%;background:rgba(255,255,255,.9);font-size:18px}.stock{position:absolute;left:7px;bottom:7px;background:rgba(255,255,255,.9);color:#198754;border-radius:999px;padding:4px 7px;font-size:11px;font-weight:700}.body{padding:9px;display:flex;flex-direction:column;flex:1}.title{font-size:13px;line-height:1.3;min-height:34px;margin-bottom:7px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}.price{font-size:17px;font-weight:800;margin-top:auto}.row{display:flex;gap:7px;margin-top:8px}.add{flex:1;border:0;border-radius:11px;padding:10px 7px;background:var(--blue);color:#fff;font-weight:750;font-size:13px}.add:active{background:var(--blue2)}.empty{text-align:center;padding:44px 10px;color:var(--muted);grid-column:1/-1}.bottom{position:fixed;left:0;right:0;bottom:0;padding:8px 10px calc(8px + env(safe-area-inset-bottom));background:color-mix(in srgb,var(--bg) 94%,transparent);backdrop-filter:blur(14px);border-top:1px solid var(--line);z-index:4}.bottom-inner{width:min(740px,100%);margin:auto;display:grid;grid-template-columns:1fr 1.45fr 1fr;gap:7px}.navbtn,.cartbtn{border:0;border-radius:14px;padding:12px 8px;font-weight:750}.navbtn{background:var(--card);color:inherit}.cartbtn{background:var(--blue);color:#fff}.badge{display:inline-grid;place-items:center;min-width:20px;height:20px;padding:0 5px;border-radius:999px;background:#fff;color:var(--blue);font-size:11px;margin-left:4px}.modal{position:fixed;inset:0;background:rgba(0,0,0,.48);display:flex;align-items:flex-end;z-index:10}.sheet{width:100%;max-height:92vh;overflow:auto;background:var(--bg);border-radius:24px 24px 0 0;padding:12px 16px calc(18px + env(safe-area-inset-bottom))}.grab{width:42px;height:5px;background:#c7d2d9;border-radius:999px;margin:0 auto 8px}.close{float:right;border:0;background:var(--card);border-radius:50%;width:36px;height:36px;font-size:22px}.sheet-img{width:100%;height:300px;object-fit:contain;background:#fff;border-radius:18px}.sheet h2{font-size:20px;line-height:1.25;margin:14px 0 7px}.sheet-price{font-size:23px;font-weight:850;margin:4px 0 10px}.description{line-height:1.5;color:var(--muted);white-space:pre-line}.submit{width:100%;border:0;border-radius:14px;padding:14px;background:var(--blue);color:#fff;font-weight:800}.cart-item{display:grid;grid-template-columns:58px 1fr auto;gap:10px;align-items:center;border-bottom:1px solid var(--line);padding:10px 0}.cart-item img{width:58px;height:58px;object-fit:contain;background:#fff;border-radius:11px}.cart-title{font-size:13px;line-height:1.25}.muted{color:var(--muted);font-size:13px}.qty{display:flex;align-items:center;gap:8px}.qty button{border:0;border-radius:10px;width:34px;height:34px;background:#e5f4fd;color:#1679b6;font-size:20px}.summary{background:var(--card);border-radius:15px;padding:13px;margin:13px 0;display:flex;justify-content:space-between;font-weight:800}.checkout input,.checkout textarea{width:100%;margin:5px 0 8px;padding:12px;border:1px solid var(--line);border-radius:12px;background:var(--card);color:inherit}.checkout textarea{min-height:70px;resize:vertical}.loader{text-align:center;padding:60px}.toast{position:fixed;left:50%;bottom:94px;transform:translateX(-50%);background:#17212b;color:#fff;padding:10px 14px;border-radius:999px;z-index:20;font-size:13px}
@media(max-width:380px){.grid{gap:7px}.shell{padding-left:7px;padding-right:7px}.body{padding:8px}.title{font-size:12px}.price{font-size:16px}.add{font-size:12px;padding:9px 5px}}
</style>
<script src="https://telegram.org/js/telegram-web-app.js"></script>
<script>
const tg=window.Telegram?.WebApp;tg?.ready();tg?.expand();
const state={products:[],filtered:[],category:'Усі',cart:{},favorites:new Set(),mode:'all',query:''};
...
function visibleProducts(){return state.products.filter(p=>{const category=state.category==='Усі'||p.category===state.category;const query=!state.query||p.title.toLowerCase().includes(state.query);const fav=state.mode!=='favorites'||state.favorites.has(p.id);return category&&query&&fav})}
function render(){
 state.filtered=visibleProducts();
 const cats=['Усі',...new Set(state.products.map(p=>p.category).filter(Boolean))].slice(0,18);
 app.innerHTML=`<div class="shell"><section class="hero"><div class="brand"><div class="logo">🍬</div><div><h1>OKVEJ</h1><p>Солодощі з доставкою по Україні</p></div></div><div class="hero-stat">${state.products.length} товарів</div></section><div class="toolbar"><div class="search-wrap"><input id="search" class="search" value="${escapeHtml(state.query)}" placeholder="Пошук товару"><span class="search-icon">🔍</span></div><div class="chips">${cats.map(c=>`<button class="chip ${c===state.category?'active':''}" data-cat="${escapeHtml(c)}">${escapeHtml(c)}</button>`).join('')}</div></div><div class="section-head"><h2>${state.mode==='favorites'?'❤️ Обране':'Каталог'}</h2><div class="count-label">${state.filtered.length} товарів</div></div><div class="grid">${state.filtered.map(card).join('')||'<div class="empty">Товарів не знайдено</div>'}</div></div><div class="bottom"><div class="bottom-inner"><button id="home" class="navbtn">🏠 Каталог</button><button id="cartOpen" class="cartbtn">🛒 Кошик <span class="badge">${count()}</span></button><button id="favorites" class="navbtn">❤️ Обране</button></div></div>`;
 document.querySelectorAll('[data-cat]').forEach(b=>b.addEventListener('click',()=>{state.category=b.dataset.cat;render()}));
 document.querySelectorAll('[data-card]').forEach(el=>el.addEventListener('click',e=>{if(e.target.closest('button'))return;details(el.dataset.card)}));
 ...
 document.getElementById('home').addEventListener('click',()=>{state.mode='all';state.category='Усі';render()});
 document.getElementById('favorites').addEventListener('click',()=>{state.mode='favorites';state.category='Усі';render()});
}
function card(p){const fav=state.favorites.has(p.id);return `<article class="card" data-card="${p.id}"><div class="photo-wrap"><img class="photo" src="${escapeHtml(p.image||'')}" alt="${escapeHtml(p.title)}"><button class="fav" data-fav="${p.id}" aria-label="Обране">${fav?'❤️':'🤍'}</button><span class="stock">● В наявності</span></div><div class="body"><div class="title">${escapeHtml(p.title)}</div><div class="price">${money(p.price)}</div><div class="row"><button class="add" data-add="${p.id}">+ У кошик</button></div></div></article>`}
...
function details(id){const p=state.products.find(x=>x.id===id);if(!p)return;const modal=document.createElement('div');modal.className='modal';modal.innerHTML=`<div class="sheet"><div class="grab"></div><button class="close">×</button><img class="sheet-img" src="${escapeHtml(p.image||'')}" alt="${escapeHtml(p.title)}"><h2>${escapeHtml(p.title)}</h2><div class="sheet-price">${money(p.price)}</div><p class="description">${escapeHtml(p.description||'Опис товару уточнюється.')}</p><button class="submit">🛒 Додати у кошик</button></div>`;document.body.appendChild(modal);modal.addEventListener('click',e=>{if(e.target===modal)modal.remove()});modal.querySelector('.close').onclick=()=>modal.remove();modal.querySelector('.submit').onclick=()=>{add(id);modal.remove()}}
function openCart(){const modal=document.createElement('div');modal.className='modal';const items=Object.values(state.cart);modal.innerHTML=`<div class="sheet"><div class="grab"></div><button class="close">×</button><h2>🛒 Кошик</h2>${items.length?items.map(x=>`<div class="cart-item"><img src="${escapeHtml(x.product.image||'')}" alt=""><div><div class="cart-title"><b>${escapeHtml(x.product.title)}</b></div><div class="muted">${money(x.product.price)}</div></div><div class="qty"><button data-minus="${x.product.id}">−</button><b>${x.qty}</b><button data-plus="${x.product.id}">+</button></div></div>`).join('')+`<div class="summary"><span>Разом</span><span>${money(total())}</span></div><div class="checkout"><input id="name" placeholder="Ім’я"><input id="phone" type="tel" placeholder="Телефон"><input id="city" placeholder="Місто"><input id="branch" placeholder="Відділення Нової пошти"><textarea id="comment" placeholder="Коментар до замовлення"></textarea><button id="order" class="submit">Оформити замовлення</button></div>`:'<div class="empty">Кошик поки порожній</div>'}</div>`;document.body.appendChild(modal);modal.addEventListener('click',e=>{if(e.target===modal)modal.remove()});modal.querySelector('.close').onclick=()=>modal.remove();modal.querySelectorAll('[data-plus]').forEach(b=>b.onclick=()=>{state.cart[b.dataset.plus].qty++;modal.remove();openCart();render()});modal.querySelectorAll('[data-minus]').forEach(b=>b.onclick=()=>{const x=state.cart[b.dataset.minus];x.qty--;if(x.qty<=0)delete state.cart[b.dataset.minus];modal.remove();openCart();render()});modal.querySelector('#order')?.addEventListener('click',()=>submitOrder(modal))}
