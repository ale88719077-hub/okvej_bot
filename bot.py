...
function home(){const c=cats();const all=Object.keys(c).sort((a,b)=>c[b]-c[a]);return `<section class="section"><div class="quick-grid"><button class="quick" data-home="catalog"><span class="ico">📂</span><b>Каталог</b><small>${state.products.length} товарів</small></button><button class="quick" data-home="favorites"><span class="ico">❤️</span><b>Обране</b><small>${state.favorites.size} товарів</small></button><button class="quick" data-home="recent"><span class="ico">🕓</span><b>Переглянуті</b><small>${state.recent.length} товарів</small></button><button class="quick" data-home="cart"><span class="ico">🛒</span><b>Кошик</b><small>${countCart()} товарів</small></button></div></section><section class="section"><div class="section-head"><h2>📂 Каталог товарів</h2><span>${all.length} категорій</span></div><div class="quick-grid"><button class="quick" data-home="catalog"><span class="ico">🛍️</span><b>Усі товари</b><small>${state.products.length} товарів</small></button>${all.map(x=>`<button class="quick" data-cat="${esc(x)}"><span class="ico">🍬</span><b>${esc(x)}</b><small>${c[x]} товарів</small></button>`).join('')}</div></section>`}
